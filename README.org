...
This is a custom implementation based on a configuration live on YouTube, subscribe to [[https://www.youtube.com/c/SystemCrafters?sub_confirmation=1][System Crafters]] (make sure to click the bell!) so that you're notified of future streams.

* My custom notes

ivy-mode 1 #This replaces builtin keybindings with counsel-equivilent
ivy is a good place to define C-l done, C-j next C-k prev etc.
helm is another completion framework contemporary with ivy.

vertico is a more modern completion framework. Not sure if it can/should fully replace ivy?

tango-dark, wombat decent built-in themes.

This block of code will allow Ctrl+l to be the only option to choose completion
...
counsel-M-x has a default of starting with initial input of a carot ^.
This (in regular expression syntax) means you'll be finding things that start with what you type.
...
#+begin_src elisp
 ...
 The default had a ^ in the ""
 
 Alt-o brings up more options when in counsel buffer

 <mode>list-mode-map usually gives keybindings etcs for a mode.
 
  When using use-package --
  - :init runs always, but before package is loaded
 ...
 M-: (requires shift key) - Brings up eval so you can type a command and execute it.

 check-parens - will find unmatched parenthases

 Package general is what allows keybinding to be defined like this:
 (spacemacs/leader-keys
     "t" '(:ignore t :which-key "toggles")
     "tt" '(<command>)

 If you get an error on install (use-package) that something doesn't exist that you know should, manually run list-packages which refreshes package list. Helpful with elpaca? Probably not.

 C-z - fully disable evil mode and go back to emacs mode. Can help if evil mode is doing something weird.
 
 C-M-\ = indent region. Good for autoformatting lips code.
